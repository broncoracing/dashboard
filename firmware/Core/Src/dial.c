...
uint8_t new_position[2] = {0,0};
uint32_t deglitch_counter[2] = {0,0};
...
void update_dial_state(uint16_t dial0, int16_t dial1){
    uint16_t adc_values[2] = {dial0, dial1};
    // Loop through each dial
    for(uint8_t dial=0; dial<2; ++dial){
        ...
        if(dial_pos != carState.dial_pos[dial]){
            ...
            if(dial_pos == new_position[dial]){
                ...
                if(deglitch_counter[dial] > DEGLITCH_COUNT) {
                    ...
                    carState.dial_pos[dial] = dial_pos;
                }
            } else {
